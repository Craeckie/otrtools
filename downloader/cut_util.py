    ...
    for frames in keyframe_list:
        ...
        if (use_frames and abs(kf_diff) <= 5) or (not use_frames and abs(kf_diff_fl) <= 0.2):
            ...
            if kf_diff > 0:
                if use_frames:
                    print(
                        "Next keyframe is just %s frames or %.3f seconds ahead, adjusting cut to be after next keyframe" % (
                        kf_diff, kf_diff_fl))
                else:
                    print(
                        "Next keyframe is just %.3f seconds ahead, adjusting cut to be after next keyframe" % kf_diff_fl)
                kf_diff += cut_offset_frames
                kf_diff_fl = + cut_offset_fl
            else:
                if use_frames:
                    print("Previous keyframe is just %s frames or %.3f seconds before, adjusting cut" % (
                    kf_diff, kf_diff_fl))
                else:
...
def parse_media_name(filename):
    ...
    filename_match = re.search(
        '(?P<name>[a-zA-Z0-9_-]+)_(?P<date>[0-9]{2}\.[0-9]{2}\.[0-9]{2})_(?P<time>[0-9]{2}-[0-9]{2})_(?P<sender>[0-9A-Za-z-]+)_(?P<duration>[0-9]+)_TVOON_DE(?P<extension>[A-Za-z0-9.]+)',
        filename)
...
def parse_base_name(filename):
    basename_match = re.search(
        '(?P<name>[a-zA-Z0-9_-]+_[0-9]{2}\.[0-9]{2}\.[0-9]{2}_[0-9]{2}-[0-9]{2}_[0-9A-Za-z-]+_[0-9]+_TVOON_DE)(?P<extension>[A-Za-z0-9.]+)',
        filename)
